...
uniform vec3 vLightDir[NUMBER_OF_LIGHTS];
...
vec4 lighting(vec3 E, vec3 N) {
    ...
    if (uLight[0].enabled) {
        if (uLight[0].position.w == 0.0) {
            // Directional Light
            result += calcDirectionalLight(0, E, normalize(uLight[0].position.xyz), N);
        } else if (uLight[0].spot_cutoff == 180.0) {
            // Point Light
            result += calcPointLight(0, E, N);
        } else {
            result += calcSpotLight(0, E, N);
        }
    }
