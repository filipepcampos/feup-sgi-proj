...
export class LiftPieceState extends InteractableGameState {
    ...
    createAnimation() {
        ...
        keyframes.push(new Keyframe(1, end));

        console.log(keyframes[0].transformation);
    ...
    update(curr) {
        if(this.animationTracker.isOver()) {
            this.stateManager.setState(new DestinationSelectionState(this.stateManager, this.gameCTO, this.renderer, this.startTile));
        } else {
